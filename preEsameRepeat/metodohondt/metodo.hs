...
merge [] xs = xs
merge (x:xs) (y:ys) 
            | numero x <= numero y    = x:merge xs (y:ys)
            | otherwise               = y:merge (x:xs) ys

sort [] = []
sort [x] = [x]
sort ps = merge (sort left) (sort right) where (left,right) = splitAt (div (length ps) 2) ps
...
daiseggioapartitox partiti p = sort ((Partito (numero p) (tail (listaVoti p))):[ps | ps <- partiti, p /= ps])
